...
tools=c("RNAup","IntaRNA","RNAplex","NUPACK","RNAcofold","pairfold","AccessFold","RNAduplex","DuplexFold","RNAhybrid","ssearch","RIsearch","bifold","bistaRNA","ractIP")
...
axis(1,at=1:15,cex.axis=1.8,las=2,labels=F)
text(1:15,-0.3,labels=tools,cex=2,srt=45)
...
axis(2,at=seq(0,1,by=1/14),labels = F,cex=1.8)
text(-0.2,seq(0,1,by=1/14),labels=rownames(cor.matrix.tools.reorder),cex=1.5,srt=0,xpd=T)

axis(1,at=seq(0,1,by=1/14),labels = colnames(cor.matrix.tools.reorder),las=2,cex.axis=1.5)
...
pdf("overall.results.heatmap.pdf",width = 12.1,height = 8)

tools_result_matrix=do.call(cbind,lapply(results_for_boxplot,function(df) return(df$V7)))
heatmap.2(tools_result_matrix,Rowv = F,margins = c(10,10),col=colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(100), cexCol = 1.8, lhei = c(2, 15),trace="none",key=F,keysize=1,key.title ="",density.info="density",cexRow = 0.4,labRow=as.character(lapply(results_for_boxplot,function(df) return(df$V8))$RIsearch))
